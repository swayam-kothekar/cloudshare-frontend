...
import { Cloud, Download, UploadCloud } from 'lucide-react';
...
const FileDownload: React.FC = () => {
  ...
  const decryptFile = async (encryptedBlob: Blob, key: Uint8Array): Promise<DecryptedFile> => {
  ...
  const extractZip = async (zipBlob: Blob): Promise<void> => {
    try {
      ...
      setStatus(`Failed to extract ZIP file: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  const handleDownload = async (): Promise<void> => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.status === 403) {
        ...
        setStatus(`File download or decryption failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  };
  ...
  const downloadAllFiles = async (): Promise<void> => {
    try {
      ...
      setStatus(`Failed to create ZIP: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
          ...
          <CardContent className="p-10">
            <div className="flex flex-col md:flex-row gap-10">
              <div className="w-full md:w-1/2 space-y-6">
                <h1 className="text-5xl font-bold mb-2 text-teal-400">You've Got Files!</h1>
                <p className="text-2xl mb-4 text-gray-300">Your secure files are ready for download.</p>
                <div className="bg-gray-700 bg-opacity-50 p-6 rounded-xl shadow-inner">
                  <h3 className="text-xl font-semibold mb-3 text-teal-300">End-to-End Encryption</h3>
                  <p className="text-gray-300">Our state-of-the-art encryption ensures your files remain private and secure throughout the entire transfer process.</p>
                </div>
                
                <div className="space-y-4">
                  <h2 className="text-2xl font-semibold text-teal-300">Share Again</h2>
              ...
              <div className="w-full md:w-1/2 border-t md:border-l md:border-t-0 border-gray-600 pt-6 md:pt-0 md:pl-10">
                <h2 className="text-3xl font-semibold mb-6 text-teal-300">Your Files</h2>
                {extractedFiles.length > 0 && (
                  <div className="space-y-6">
                    <ul className="space-y-3 max-h-96 overflow-y-auto pr-4 custom-scrollbar">
                      {extractedFiles.map((file, index) => (
                        <li key={index} className="flex justify-between items-center bg-gray-700 bg-opacity-50 p-4 rounded-xl hover:bg-opacity-70 transition-all duration-200">
                          <span className="truncate mr-3 text-gray-300">{file.name}</span>
                    ...
                    <Button onClick={downloadAllFiles} className="w-full bg-teal-500 hover:bg-teal-600 text-white py-3 text-lg">
                      <Download className="mr-2 h-5 w-5" /> Download All
                    </Button>
                  </div>
                )}
                {status && <p className="text-teal-400 mt-6 text-center">{status}</p>}
              </div>
            </div>
          </CardContent>
        </Card>
